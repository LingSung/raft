...
func (r *Raft) appendEntries(req *pb.AppendEntriesRequest) (*pb.AppendEntriesResponse, error) {
	...
	if prevLogId != 0 && prevLogTerm != 0 {
		...
		myPrevLog := r.getLog(prevLogId)

		if prevLogTerm != myPrevLog.GetTerm() {
			r.logger.Info("the given previous log from leader is missing or mismatched",
				...
				zap.Uint64("logTerm", myPrevLog.GetTerm()))
			return &pb.AppendEntriesResponse{Term: r.currentTerm, Success: false}, nil
		}
	...
	if req.GetLeaderCommitId() > r.commitIndex {
		myLastLogId, _ := r.getLastLog()
		if myLastLogId < req.GetLeaderCommitId() {
			r.setCommitIndex(myLastLogId)
		} else {
...
func (r *Raft) requestVote(req *pb.RequestVoteRequest) (*pb.RequestVoteResponse, error) {
	...
	myLastLogId, myLastLogTerm := r.getLastLog()
	if myLastLogTerm > req.GetLastLogTerm() || (myLastLogTerm == req.GetLastLogTerm() && myLastLogId > req.GetLastLogId()) {
		// TODO: (A.7) - if votedFor is null or candidateId, and candidate’s log is at least as up-to-date as receiver’s log, grant vote
...
func (r *Raft) broadcastAppendEntries(ctx context.Context, appendEntriesResultCh chan *appendEntriesResult) {
	...
	for peerId, peer := range r.peers {
		...
		peerPrevLog := r.getLog(r.nextIndex[peerId] - 1)
		peerEntries := r.getLogs(r.nextIndex[peerId])

		req := &pb.AppendEntriesRequest{
			Term:           r.currentTerm,
			LeaderId:       r.id,
			LeaderCommitId: r.commitIndex,
			PrevLogId:      peerPrevLog.GetId(),
			PrevLogTerm:    peerPrevLog.GetTerm(),
			Entries:        peerEntries,
		}
		r.logger.Debug("send append entries", zap.Uint32("peer", peerId), zap.Any("request", req), zap.Int("entries", len(peerEntries)))
...
func (r *Raft) handleAppendEntriesResult(result *appendEntriesResult) {
	...
	for i := len(logs) - 1; i >= 0; i-- {
		...
		replicas := 1
		currentCommitId := r.commitIndex + 1 + uint64(i)
		for peerId := range r.peers {
			peerId := peerId

			if r.matchIndex[peerId] >= currentCommitId {
				replicas++
			}
		}

		if replicas >= replicasNeeded {
			r.setCommitIndex(currentCommitId)
			go r.applyLogs(r.applyCh)
